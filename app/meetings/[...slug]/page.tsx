...
import { GET_MEETING_BY_PATH } from '@/lib/queries'
...
interface MeetingByPathData {
  route: {
    entity: DrupalMeeting
  } | null
}

async function getMeeting(path: string): Promise<DrupalMeeting | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<MeetingByPathData>({
      query: GET_MEETING_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/meetings/${slug.join('/')}`
  const item = await getMeeting(path)

  if (!item) {
    return { title: 'Meeting Not Found | County' }
  }
              ...
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  ...
                  {(item as any).agendaUrl && (
                    <div>
                      <dt className="text-sm text-gray-500">Agenda URL</dt>
